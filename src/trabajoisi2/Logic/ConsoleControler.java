...
//                    System.out.println("Comando recibido desde la consola: " + input);
    ...
    private static void addPelicula(){
        ...
        try {
            ...
            SuscriptionType = SuscriptorFactory.createSuscriptor(type);
            System.out.println("Bien! Ahora añadamos una descripcion interesante para llamar la atencion de los usarios");
            StringBuilder parrafo = new StringBuilder();
                String linea;
                while (!(linea = consoleReader.readLine()).isEmpty()) {
                    parrafo.append(linea).append("\n");
                }
            Descripcion = parrafo.toString();
        } catch (IOException ex) {
            Logger.getLogger(ConsoleControler.class.getName()).log(Level.SEVERE, null, ex);
        }
        Fachada.getInstance().agregarContenido(new Pelicula(Name,Duracion,Genero,SuscriptionType, Descripcion));
    }
    ...
    private static void addSerie(){
        ...
        try{
            ...
            String type = consoleReader.readLine();
            SuscripcionType = SuscriptorFactory.createSuscriptor(type);
            System.out.println("Bien! Ahora añadamos una descripcion interesante para llamar la atencion de los usarios");
            StringBuilder parrafo = new StringBuilder();
                String linea;
                while (!(linea = consoleReader.readLine()).isEmpty()) {
        ...
        Fachada.getInstance().agregarContenido(new Serie(Nombre, NumCapitulos, Genero, SuscripcionType,Descripcion));
    }
}
